...
It's possible that bias in healthcare can be remedied, but I'm unaware of any project that has improved unconcious racial bias at such a large scale in such a short period of time.
...
If we're certain that these are the only factors, and we're also fairly certain that they can't be changed within the necessary timespan, what's the point of studying the matter? 
...
Unlike other potential therapies, we already have a lot of vitamin D clinical research data and we know that it's safe to consume prophylatically.
Vitamin D would also go a long way to explain why people of colour are dying at such high rates in the northern hemiphere where sunlight is currently scarce, but not in equatorial or southern parts of the world where sunlight is currently abundant.
While skin colour is not the only factor that changes serum vitamin D, and race is a social category that's only loosely related to skin colour, it is the case that in most northern latitudes people with darker skin tend to have lower vitamin D levels in the winter. [#TODO add study]
...
We might respond to this claim by pointing out that different racial groups live in different neighborhoods and go to different schools that are allocated different resources, and this probably changes how prepared students are for the test.
If you control for those factors, you tend to find that race provides no real information about SAT scores because it's just a proxy for which school you go to. [#TODO add study]
The true causal factors are related to the school, not to the race of the student. 

By contrast, when you do the same exercise about arrest decisions you probably would find that race is an independent factor.
Even if you account for neighborhood, crime, or time of day, you still find that race is an important predictor of arrest because police, consciously or not, tend to arrest people of colour more often.
So when you include race in an arrest model, you tend to find that race provides some unique and important information, because racism causes arrest. [#TODO add study]

We should have a strong intuition that the virus itself is not racist.  [#TODO why? Is this a good place to say something like "because a virus cares about physical traits, and race is a socially constructed concept" and some reference to disproven race science? Or maybe the piece about race being an inadequate data proxy for skin colour?]
...
it's not like people of colour in northern latitudes (but not southern latitudes) share some genetic factor that the virus attacks.
Instead, what we're looking for is something like the SAT case: if we include all the causal variables in our model, we assume that we will discover that race doesn't really add much. 
If we find that race still provides a lot of information (meaning variation not accounted for by any of the other factors), that indicates that we're probably leaving out a causal factor. 
...
The main outcome [#TODO find a different word] of this study was positive Covid diagnosis, 
...
It directly controls for pre-existing conditions, but there also is a kind of implicit control on medical bias because the main outcome [#TODO use the new word here too] was a positive test.
...
Each of these biased decisions endangers mother and child, and it's a big part of why there's a big racial gap in infant mortality. [#TODO find study]

By contrast, Covid diagnosis really only involves the one decision of whether to test or not, and we can assume that racial bias would tend to lead doctors to undertest Black people. [#TODO find study, could be about undertesting for other disease]
...
A [recent study](https://nutrition.bmj.com/content/early/2020/05/20/bmjnph-2020-000096) in the BMJ [#TODO expand acronym] provides a strong argument that the missing factor is vitamin D. 
...
In this case we know that people with darker skin tend to have lower vitamin D levels than people with lighter skin[#TODO find study], and that that gets worse as you move north. 
In places like Florida, it doesn't really matter how efficient your skin is at making vitamin D because it's sunny most of the year. 
...
![](/img/excess_aa_mortality.jpg) [#TODO where is this graph from?]
...
If excess infant mortality were higher in northern states than in southern ones, that would suggest that latitude acted on social determinants of health. [#TODO why?]
This data from the CDC [#TODO link to where you are getting this data] shows that children of Black or African-American mothers die at a rate 2-3 times higher than the children of white mothers, and this rate doesn't really change by latitude. 

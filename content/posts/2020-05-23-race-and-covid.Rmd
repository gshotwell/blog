...
When you think about it, this model is pretty fatalistic because none of these factors can really be changed during the timeline of the pandemic. 
We're probably not going to make major changes to our housing or economy such that Black people are not disproportionately proximate to the virus, and it's unlikely that diabetes or hypertension rates are going to drop over the next six months.
...
Even if these are major causes, they're not causes that we can modify over the next year.

I think this pretty dangerous because we don't know that much about Covid, so it's possible that this isn't a complete causal model and there are things that we can change which reduce excess mortality among people of colour. 
If we have too much certainty that this excess mortality is entirely composed of these social determinants of health, 
then researchers won't invest time or money trying to figure out things that can be treated.
If the legacies of slavery and racism compeletely determines racial health outcomes, why should we look for other causes? 

# The vitamin D model

I've been very interested in vitamin D deficiency as a potential cause of Covid-19 mortality because it's one of the few substances that can be widely and safely taken by lots of people.
Unlike other potential therapies, we have a lot of vitamin D, and we know that it's safe to consume prophylatically.
One of the things that vitamin D would explain is why people of colour are dying at such high rates in the Northern hemiphere where sunlight is scarce, but not equitorial or southern parts of the world where sunlight is abundant.
I've gotten a fair bit of criticism about this from various sources that this suggestion is racist because it undermines the idea that social factors are only true causes of excess mortality among people of colour. 
In other words people think I'm arguing for a model which says that vitamin D is the sole cause of excess mortality:
...
This isn't an unreasonable thing to think, but neither myself nor any actual vitamin D scientists are suggesting that model.
...
The reason to focus on vitamin D is not that it's the most important cause, but that it's the best treatment candidate.
It's a cheap, safe therapy, that doesn't require big systemic change,
so even if it's only responsible for 1% of the excess mortality it's worth investing in.
We can administer vitamin D safely to people in nursing homes, or institutional settings without any real risk, 
and there really aren't any other potential prophylatic Covid treatments that have that quality.
It would be great if vitamin D were a factor, because it's a factor we can change over the next year.
...
What they found is that after controlling for all of these things Black, Asian, and mixed race people had about twice the risk of dying of Covid relative to white people.
...
So bias in this case would reinforce, rather than undermine the finding that Black and Hispanic people are at a higher risk of Covid-19 diagnosis. 
...
If you have information about something which you know is related to vitamin D status, then you can include that as a proxy. 
...
This is a pretty goo argument for vitamin D's involvemnt, because it's hard to find a story where latitude modifies other inputs into the Covid mortality.
...
This data from the CDC is really depressing and shows that infant mortality among African Americans is 2-3 times higher than among white people in just about every state.
...
which is good news because that means there's something we can do about it.

...
This comes up for me personally, when coworkers tell me some version of "Hey, your work is great, you're an excellent developer, but have you thought of switching over the Python/Scala/Javascript so that you can really make a contribution." 
...
By contrast, most of the very senior engineers I've met understand that all programming languages are basically just bundles of trade-offs, and so no single language is going to be globally superior to another.
...
Overall I think there are basically four main features of the core R language which are essential to my work. 
These are things which are present in R, and which I haven't found to be available or accessible in any other single language, and which make R the best choice for my work:

1) Data science structures in the base language
1) Non-standard evaluation
1) Packaging consensus
...
Data structures in the base language tend to be more stable than thoses provided by external dependencies. For instance, I can run base R code from 2010 and be reasonably sure that the code will behave the same way today as it did a decade ago, because maintainers of the core  language are very conservative in introducing breaking changes. 

This is probably not true of R or Python code which relies on libraries like dplyr or pandas because both of those libraries prioritize feature improvement over stability. 
This isn't meant as a criticism of either of those libraries, but just to point out that it is beneficial to use a language  that  allows you to do meaningful  data science work without importing external libraries. 
...
These people will  make the  same basic argument about pandas that pandas people make about  R: That it's  something appropriate for research, but production code requires vanilla Python. Switching from R to Python often doesn't significantly reduce deployment friction, because you still have to do some kind of microservice process in order to isolate the data science dependencies from the rest of the Python code base. 
...
It shouldn't be the first thing you reach for and it's very dangerous if you don't know what your doing, 
but it allows you to solve problems which would  otherwise be unsolvable. 
...
Using NSE, the package translates captures the expressions supplied by the user and translates them into programmatic representations that are understood by the various backends.
...
One of the great things about accessing the user's environment is that there's a wealth of information in that environment which is particularly meaningful to that user. 
...
You might think that it's too much effort to write a friendly error message, but in my work I find that details like this help build delightful products which are easy to use. 
...
In hindsight this was a very important morning for me because in many ways it shaped the course ofmy career for the next ten years. 
I probably had about 20 minutes to become interested and excited about the project. 
After all I had lots of homework to do, and programming wasn't something that was on any of my to-do lists at the time. 
...
If there are any Python evangelists still reading this, they might have a response to this that begins with, "Well you just," but remember the user that I care the most about only has 20 minutes of attention and no real programming skill, so the only thing they can "just" do is copy and paste one line of code into a console.
...
When I'm building a statistical model, what I'm really doing is creating a mapping between some set of inputs and an outpu; in other words, a function. 

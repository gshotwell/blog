...
Data science students are often told that the most important thing they can learn is how to use SQL.
...
Moreover how a specific database is set up will dramatically change which queries are optimal for that database.
...
This is usually good advice because it leads to more readable SQL code, but can lead to [600% higher costs](https://medium.com/@AtheonAnalytics/snowflake-query-optimiser-unoptimised-cf0223bdd136) when run against some databases.
...
The idea is that the user should write a query which is understandable to them, and the database should take care of responding to that query in an efficient manner.

# Writing dbplyr wrappers

A big chunk of my job over the last few years has been writing database wrappers to perform common queries, and I try to use dbplyr for these functions as much as possible.
...
For example let's say that we wanted to write a function to get the mean arrival time per airport.
...
mean_dest_time_sql <- function(min_time, max_time, con = con) {
  where_clause <- paste0('WHERE ("air_time" >= ', min_time, ') AND ("air_time" <= ', max_time, ')')
...
Writing functions like this is annoying because you spend a lot of time pasting queries together, but the bigger issue is that users will never stop asking for more additions and your function will gain a million arguments.
...
Writing dbplyr functions lets you leverage all of the dbplyr infrastructure which means that you have less code to maintain and less education to do.

### Composable database wrappers 

The second main reason to use dplyr is that it lets you write composable SQL function.
...
For example putting the filter and mutate in different places will generate different SQL, but both queries will return the same data.
...
You can't write small fragments of queries and have them stack together in a predictable fashion and have them work.
As a result when you write wrappers around query construction you end up trying to build a comprehensive function that limits the user to the things that you had in mind when you wrote it.
...
I've been writing dbplyr wrappers for some time, but I've only recently come up with a testing pattern which makes me happy.
There are three main things that I want when testing database functions:
...
My new approach is to record a mock of the whole database and store that as an on-disk [duckdb database](https://duckdb.org/).
This means that I can test many functions using the same mocked data, and the data is easier to understand.
In the test files I point my functions to the new database and run two types of tests:

1.  Test that the function generates the expected SQL using the

2.  Test that the function produces the right output

For example I would test that the `by_day` function produced the right output with a test like this:

```{r, "by_day output test"}
library(testthat)
...
This gives future R developers a clear understanding of what this functions is supposed to do, which lets them make changes with the confidence that they won't change the user expectations.
But I also want to generate SQL so that I can use it to commmunicate with the people who maintain the database itself.
...
When you run the test suite for the first time this will generate the expected SQL query which will be used to test the function in the future:
...
For example if you noticed odd results when running the query against the actual database you can send them the specific query which no longer produces the expected output.
...
No programming framework is totally comprehensive and there are plenty of times where it's important to move past dbplyr and optimize the actual SQL that your functions generate.
